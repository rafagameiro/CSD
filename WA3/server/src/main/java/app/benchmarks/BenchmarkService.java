...
public class BenchmarkService {
    ...
    public void testRegisterUsers() {
        ...
        for (int i = 0; i < operations; i++) {
            ...
            try {
                walletService.createUser(new User(username, password));
            } catch (Exception e) {
    ...
    public void testDeposit() {
        ...
        for (int i = 0; i < operations; i++) {
            ...
            Long start = System.currentTimeMillis();
            try {
                walletService.deposit(username, new Amount((float) i + 1));
            } catch (Exception e) {
                System.out.println(e.getMessage());
            }
            Long end = System.currentTimeMillis();
    ...
    public void testTransfer() {
        ...
        for (int i = 0; i < operations; i++) {
            ...
            Long start = System.currentTimeMillis();
            try {
                walletService.transfer(new Transaction(UUID.randomUUID(), from, to, i + 1));
            } catch (Exception e) {
                System.out.println(e.getMessage());
            }
    ...
    public void testLedger() {
        ...
        for (int i = 0; i < operations; i++) {
            if (i % 2 == 0) {
                Long start = System.currentTimeMillis();
                try {
                    walletService.getLedger(registeredUsers.get(i % registeredUsers.size()));
                } catch (Exception e) {
                    System.out.println(e.getMessage());
                }
                Long end = System.currentTimeMillis();
